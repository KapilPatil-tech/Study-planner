const state = {
  ...
  user: null,
  data: { subjects: [], assignments: [], exams: [], timetable: [], notes: [], attendance: [] },
  admin: { stats: null, users: [], records: { subjects: [], assignments: [], exams: [], attendance: [] }, tab: "students", analytics: null, search: "" },
  page: "dashboard",
  edit: null
};

const $ = id => document.getElementById(id);
const authView = $("authView"), appView = $("appView"), modal = $("modal");
...
function setAuthTab(tab) {
  document.querySelectorAll(".auth-tabs button").forEach(b => b.classList.toggle("active", b.dataset.auth === tab));
  $("loginForm").classList.toggle("hidden", tab !== "login");
  $("signupForm").classList.toggle("hidden", tab !== "signup");
}
document.querySelectorAll("[data-auth]").forEach(b => b.onclick = () => setAuthTab(b.dataset.auth));
...
$("signupForm").onsubmit = async e => {
  e.preventDefault();
  try {
    const result = await api("/api/auth/signup", { method: "POST", body: JSON.stringify({
      name: $("signupName").value, rollNo: $("signupRoll").value,
      email: $("signupEmail").value, password: $("signupPassword").value,
      college: $("signupCollege").value, branch: $("signupBranch").value,
...
$("themeBtn").onclick = () => {
  ...
  localStorage.setItem("esp_theme", document.body.classList.contains("dark-mode") ? "dark" : "light");
};
if (localStorage.getItem("esp_theme") === "dark") document.body.classList.add("dark-mode");
...
$("pdfBtn").onclick = async () => {
  try {
    const res = await fetch("/api/export/pdf", { headers: { Authorization: `Bearer ${state.token}` } });
    if (!res.ok) throw new Error("Could not create PDF");
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "Engineering_Study_Report.pdf"; a.click();
    URL.revokeObjectURL(url);
  } catch (e) { toast(e.message); }
};

$("backupBtn").onclick = () => {
  const payload = { exportedAt: new Date().toISOString(), user: state.user, ...state.data };
...
function formatDate(d) { if (!d) return "-"; const x = new Date(d + "T00:00:00"); return isNaN(x) ? d : x.toLocaleDateString(); }
...
function loadSavedPlan() {
  try { return JSON.parse(localStorage.getItem(plannerStorageKey()) || "null"); } catch { return null; }
}
...
function generateSmartPlan(opts) {
  ...
  const exam = opts.examId ? d.exams.find(x => String(x._id) === String(opts.examId)) : null;
  ...
  const totalDays = Math.max(1, Math.min(60, plannerDaysBetween(today,end)+1));
  const hours = Math.max(1, Math.min(12, Number(opts.hours) || 3));
  const session = Math.max(25, Math.min(120, Number(opts.session) || 50));
  const weak = (opts.weak || "").split(",").map(s=>s.trim().toLowerCase()).filter(Boolean);

  const upcomingAssignments = d.assignments.filter(a=>!a.completed).map(a=>({
    subject:String(a.subject||"").toLowerCase(), date:plannerDate(a.dueDate), priority:a.priority
  }));
  const upcomingExams = d.exams.map(e=>({subject:String(e.subject||"").toLowerCase(), date:plannerDate(e.date)}));

  const subjects = d.subjects.length ? d.subjects.map(s=>({
    name:s.name || "General Study",
    score:
      (weak.some(w => String(s.name||"").toLowerCase().includes(w)) ? 5 : 0) +
      (s.completed ? 0 : 2) +
      (upcomingAssignments.some(a=>a.subject.includes(String(s.name||"").toLowerCase()) && a.date && plannerDaysBetween(today,a.date)<=7) ? 3 : 0) +
      (upcomingExams.some(e=>e.subject.includes(String(s.name||"").toLowerCase()) && e.date && plannerDaysBetween(today,e.date)<=14) ? 4 : 0)
  ...
  for(let dayIndex=0; dayIndex<totalDays; dayIndex++){
    ...
    for(let s=0;s<targetSessions;s++){
      ...
      const mins = Math.min(session, available - slots.reduce((sum,x)=>sum+x.minutes,0));
      if(mins<20) break;
      const isRevision = isExamEve || dayIndex >= totalDays-2;
      slots.push({
        subject:subject.name,
        minutes:mins,
        type:isRevision ? "Revision & practice" : (s===0 ? "Concept study" : "Practice & questions"),
        done:false
      });
      completedMinutes += mins;
    }
    if(!slots.length) slots.push({subject:subjects[0].name,minutes:session,type:"Concept study",done:false});
    plan.push({date:date.toISOString().slice(0,10),label:date.toLocaleDateString(undefined,{weekday:"long",day:"numeric",month:"short"}),sessions:slots});
  }
  return {
...
function localKey(name){ return `esp_${name}_${state.user?._id || state.user?.id || "student"}`; }
function getLocal(name, fallback=[]){ try { return JSON.parse(localStorage.getItem(localKey(name)) || JSON.stringify(fallback)); } catch { return fallback; } }
function setLocal(name, value){ localStorage.setItem(localKey(name), JSON.stringify(value)); }
...
function examPrepKey(examId){ return `${localKey("exam_prep")}_${examId}`; }
function getPrep(examId){
  try { return JSON.parse(localStorage.getItem(examPrepKey(examId)) || "null"); } catch { return null; }
}
function savePrep(examId,obj){ localStorage.setItem(examPrepKey(examId),JSON.stringify(obj)); }

function ensureNotifications(){
  const existing=getLocal("notifications",[]);
  const generated=[];
  const now=new Date(); now.setHours(0,0,0,0);
  state.data.assignments.filter(a=>!a.completed).forEach(a=>{
    const n=daysUntil(a.dueDate);
    if(n!==null && n<=2) generated.push({id:`a_${a._id}`,title:`Assignment due ${n<=0?"today":n===1?"tomorrow":"soon"}`,body:`${a.title}${a.subject?` · ${a.subject}`:""}`,kind:"deadline",read:false,createdAt:new Date().toISOString()});
  });
  state.data.exams.forEach(e=>{
    const n=daysUntil(e.date);
    if(n!==null && n>=0 && n<=7) generated.push({id:`e_${e._id}`,title:`Exam in ${n===0?"today":n===1?"1 day":`${n} days`}`,body:`${e.subject}${e.examType?` · ${e.examType}`:""}`,kind:"exam",read:false,createdAt:new Date().toISOString()});
  });
  state.data.attendance.forEach(a=>{
    const p=pct(safeNum(a.attended),safeNum(a.total));
    if(a.total && p<75) generated.push({id:`att_${a._id}`,title:"Attendance critical",body:`${a.subject} is at ${p}% attendance.`,kind:"attendance",read:false,createdAt:new Date().toISOString()});
  });
  const map=new Map(existing.map(x=>[x.id,x]));
  generated.forEach(x=>{ if(!map.has(x.id)) map.set(x.id,x); });
  const result=[...map.values()].sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)).slice(0,100);
  setLocal("notifications",result); return result;
}
function goalsData(){ return getLocal("goals",{dailyMinutes:120,weeklyMinutes:600,completedMinutes:0,streak:0,lastStudyDate:"",history:[]}); }
function saveGoals(x){ setLocal("goals",x); }
function studyMinutesFromPlan(){
  const p=loadSavedPlan(); return p ? p.plan.flatMap(d=>d.sessions).filter(s=>s.done).reduce((a,s)=>a+safeNum(s.minutes),0) : 0;
}
function calculateExamPrep(exam){
  const key=exam._id, existing=getPrep(key);
  const topics=(existing?.topics||String(exam.syllabus||"").split(/[\n,;]+/).map(x=>x.trim()).filter(Boolean).map((name,i)=>({id:`t${i}`,name,done:false,important:i<2})));
  return {topics,notes:existing?.notes||"",questions:existing?.questions||0,score:existing?.score||0};
}
function prepScore(prep){
  const t=prep.topics||[]; return t.length?Math.round(t.filter(x=>x.done).length/t.length*100):0;
}
function generateNotificationsFromData(){ ensureNotifications(); }

function render() {
  const titles = {
    dashboard:["OVERVIEW","Dashboard"], subjects:["ACADEMICS","Subjects"],
    assignments:["PRODUCTIVITY","Assignments"], exams:["ACADEMICS","Exam Schedule"],
    timetable:["SCHEDULE","Timetable"], notes:["KNOWLEDGE","Notes"],
    attendance:["ANALYTICS","Attendance"], "attendance-intelligence":["ANALYTICS","Attendance Intelligence"], planner:["AI STUDY TOOLS","Smart Study Planner"],
    "exam-prep":["EXAM TOOLS","Exam Preparation"],
    analytics:["ANALYTICS","Study Analytics"],
    notifications:["ALERTS","Notifications"],
    goals:["PRODUCTIVITY","Goals & Streak"],
    profile:["ACCOUNT","Profile"], admin:["ADMINISTRATION","Admin Panel"]
  };
  ...
  if (state.user?.role !== "admin" && state.page === "admin") state.page = "dashboard";
...
dashboard() {
  ...
  const attendanceAvg = attendance.length
    ? Math.round(attendance.reduce((sum, x) => sum + pct(Number(x.attended), Number(x.total)), 0) / attendance.length)
    : 0;
  const productivity = Math.round((subjectProgress + assignmentProgress + attendanceAvg) / 3);
  ...
  const examItems = exams
    ...
    .sort((a,b) => a._date-b._date)
    .slice(0,5);
  const upcoming = [...deadlineItems, ...examItems].sort((a,b)=>a._date-b._date).slice(0,6);
  ...
  const subjectCards = subjects.slice().sort((a,b)=>Number(b.completed)-Number(a.completed)).slice(0,6);
  const firstName = String(state.user?.name || "Student").trim().split(/\s+/)[0] || "Student";
  ...
  return `
    ...
    <div class="dashboard-grid-main">
      ...
      <div class="card dashboard-panel">
        ...
        ${upcoming.length ? `<div class="deadline-list">${upcoming.map(x=>{const left=daysLeft(x._date); const urgent=left<=2; return `<div class="deadline-row"><div class="deadline-icon ${urgent?"urgent":""}">${x._kind==="Exam"?"🗓️":"📝"}</div><div class="deadline-info"><b>${esc(x.title || x.subject)}</b><span>${esc(x._kind)}${x.examType?" · "+esc(x.examType):""} · ${formatDate(String(x._date.toISOString()).slice(0,10))}</span></div><span class="countdown ${urgent?"urgent":""}">${left===0?"Today":left===1?"Tomorrow":left+" days"}</span></div>`}).join("")}</div>` : empty("No upcoming deadlines or exams.")}
      </div>
    </div>
...
subjects() {
  return listPage("Subjects","Manage your semester subjects.","Add Subject","subjects",
    state.data.subjects.map(x=>`
      <div class="item">
...
assignments() {
  return listPage("Assignments","Track labs, submissions, projects and homework.","Add Assignment","assignments",
    state.data.assignments.map(x=>`
      ...
      <div class="actions"><button class="small-btn" data-toggle="assignments" data-id="${x._id}">${x.completed?"Undo":"Complete"}</button><button class="small-btn" data-edit="assignments" data-id="${x._id}">Edit</button><button class="small-btn danger" data-delete="assignments" data-id="${x._id}">Delete</button></div></div>`).join(""));
},
exams() {
  return listPage("Exam Schedule","Keep internal, practical, mid-sem and end-sem exams organized.","Add Exam","exams",
    state.data.exams.map(x=>`<div class="item"><div class="item-main"><div class="item-title">${esc(x.subject)}</div><div class="item-meta">${esc(x.examType||"Exam")} · ${formatDate(x.date)} · ${esc(x.time||"")} · Room ${esc(x.room||"-")}</div><div class="item-meta">${esc(x.syllabus||"")}</div></div><div class="actions"><button class="small-btn" data-edit="exams" data-id="${x._id}">Edit</button><button class="small-btn danger" data-delete="exams" data-id="${x._id}">Delete</button></div></div>`).join(""));
},
timetable() {
  return listPage("Timetable","Build your weekly class schedule.","Add Class","timetable",
    state.data.timetable.map(x=>`<div class="item"><div class="item-main"><div class="item-title">${esc(x.day)} · ${esc(x.subject)}</div><div class="item-meta">${esc(x.startTime)} - ${esc(x.endTime)} · Room ${esc(x.room||"-")} · ${esc(x.faculty||"")}</div></div><div class="actions"><button class="small-btn" data-edit="timetable" data-id="${x._id}">Edit</button><button class="small-btn danger" data-delete="timetable" data-id="${x._id}">Delete</button></div></div>`).join(""));
},
notes() {
  return listPage("Notes","Store revision points, formulas, lab notes and reminders.","Add Note","notes",
    state.data.notes.map(x=>`<div class="item"><div class="item-main"><div class="item-title">📝 ${esc(x.title)}</div><div class="item-meta">${esc(x.subject||"")} · Updated ${new Date(x.updatedAt).toLocaleDateString()}</div><p style="margin-top:8px;font-size:13px">${esc(x.content||"").slice(0,180)}</p></div><div class="actions"><button class="small-btn" data-edit="notes" data-id="${x._id}">Edit</button><button class="small-btn danger" data-delete="notes" data-id="${x._id}">Delete</button></div></div>`).join(""));
},
attendance() {
  const rows = state.data.attendance.map(x=>{const p=pct(x.attended,x.total); return `<tr><td>${esc(x.subject)}</td><td>${x.attended}</td><td>${x.total}</td><td><b>${p}%</b><div class="progress" style="margin-top:6px"><i style="width:${p}%"></i></div></td><td><button class="small-btn" data-edit="attendance" data-id="${x._id}">Edit</button><button class="small-btn danger" data-delete="attendance" data-id="${x._id}">Delete</button></td></tr>`}).join("");
...
"attendance-intelligence"() {
  ...
  const avg = rows.length ? Math.round(rows.reduce((a,x)=>a+pct(Number(x.attended),Number(x.total)),0)/rows.length) : 0;
  const critical = rows.filter(x=>Number(x.total)>0 && pct(Number(x.attended),Number(x.total))<75).length;
  const healthy = rows.filter(x=>Number(x.total)>0 && pct(Number(x.attended),Number(x.total))>=80).length;
  ...
  <div class="grid-2">
    <div class="card">
      ...
      ${rows.length ? `<div class="attendance-bars">${rows.map(x=>{
        const a=Number(x.attended), t=Number(x.total), q=pct(a,t);
        const risk=q<75?"Critical":q<80?"Warning":"Healthy";
      ...
      }).join("")}</div>` : empty("No attendance records yet.") }
    </div>
...
admin() {
  if (state.user?.role !== "admin") return `<div class="card">${empty("Admin access required.")}</div>`;
  ...
  if (state.admin.tab === "students") {
    ...
    content = adminRecordTable(state.admin.records.subjects, ["Subject","Student","Code","Faculty","Deadline","Status"], x => `<td><b>${esc(x.name)}</b></td><td>${esc(x.userId?.name||"-")}</td><td>${esc(x.code||"-")}</td><td>${esc(x.faculty||"-")}</td><td>${formatDate(x.deadline)}</td><td>${x.completed?"Completed":"Pending"}</td>`);
  } else if (state.admin.tab === "assignments") {
    content = adminRecordTable(state.admin.records.assignments, ["Assignment","Student","Subject","Due","Priority","Status"], x => `<td><b>${esc(x.title)}</b></td><td>${esc(x.userId?.name||"-")}</td><td>${esc(x.subject||"-")}</td><td>${formatDate(x.dueDate)}</td><td>${esc(x.priority)}</td><td>${x.completed?"Completed":"Pending"}</td>`);
  } else if (state.admin.tab === "attendance") {
    content = adminRecordTable(state.admin.records.attendance, ["Subject","Student","Attended","Total","Percentage"], x => `<td><b>${esc(x.subject)}</b></td><td>${esc(x.userId?.name||"-")}</td><td>${x.attended}</td><td>${x.total}</td><td><b>${pct(x.attended,x.total)}%</b></td>`);
  } else {
    content = adminRecordTable(state.admin.records.exams, ["Subject","Student","Type","Date","Time","Room"], x => `<td><b>${esc(x.subject)}</b></td><td>${esc(x.userId?.name||"-")}</td><td>${esc(x.examType||"-")}</td><td>${formatDate(x.date)}</td><td>${esc(x.time||"-")}</td><td>${esc(x.room||"-")}</td>`);
  }
...
planner() {
  ...
  const exams=state.data.exams.filter(x=>plannerDate(x.date)).sort((a,b)=>plannerDate(a.date)-plannerDate(b.date));
  const today=new Date(); today.setHours(0,0,0,0);
  const upcoming=exams.filter(x=>plannerDate(x.date)>=today);
  const examOptions=upcoming.map(x=>`<option value="${esc(x._id)}" ${saved?.examId===x._id?"selected":""}>${esc(x.subject)} — ${formatDate(x.date)}${x.examType?` (${esc(x.examType)})`:""}</option>`).join("");
  const defaultHours=saved?.hours||3, defaultSession=saved?.session||50, defaultDays=saved?.days||7, defaultWeak=saved?.weak||"";
  const examCountdown=saved?.examDate ? Math.max(0,plannerDaysBetween(today,plannerDate(saved.examDate))) : null;
  const planHtml=saved ? saved.plan.map((day,di)=>`
    <div class="plan-day">
      ...
      <div class="plan-sessions">
        ${day.sessions.map((s,si)=>`<label class="study-session ${s.done?"done":""}">
    ...
    </div>`).join("") : empty("Generate your first personalized study plan.");
  ...
  <div class="grid-2 planner-grid">
    ...
    <div class="card">
      ...
      ${saved.examSubject?`<div class="planner-exam">🗓️ <b>${esc(saved.examSubject)}</b> exam on <b>${formatDate(saved.examDate)}</b></div>`:""}`:empty("Generate a plan to see your schedule, countdown and completion progress.")}
    </div>
  </div>
...
"exam-prep"() {
  const exams=state.data.exams.filter(e=>daysUntil(e.date)!==null).sort((a,b)=>daysUntil(a.date)-daysUntil(b.date));
  const selected=state._selectedExamPrep || exams[0]?._id || "";
  const exam=exams.find(e=>e._id===selected) || exams[0];
  if(!exam) return `<div class="card">${empty("Add an exam first to start exam preparation.")}</div>`;
  const prep=calculateExamPrep(exam), score=prepScore(prep), left=daysUntil(exam.date);
...
analytics() {
  const d=state.data;
  const att=d.attendance.map(x=>({subject:x.subject,p:pct(safeNum(x.attended),safeNum(x.total))}));
  const subjectRows=d.subjects.map(s=>`<div class="analytics-row"><span>${esc(s.name)}</span><div class="progress"><i style="width:${s.completed?100:0}%"></i></div><b>${s.completed?100:0}%</b></div>`).join("");
  const completedA=d.assignments.filter(x=>x.completed).length;
  const completedS=d.subjects.filter(x=>x.completed).length;
  const avg=att.length?Math.round(att.reduce((a,x)=>a+x.p,0)/att.length):0;
  const doneSessions=loadSavedPlan()?.plan.flatMap(x=>x.sessions).filter(x=>x.done).length||0;
...
goals() {
  const g=goalsData(), today=new Date().toISOString().slice(0,10);
  const daily=Math.min(100,Math.round((safeNum(g.completedMinutes)/Math.max(1,safeNum(g.dailyMinutes)))*100));
  const weekly=Math.min(100,Math.round((safeNum(g.completedMinutes)/Math.max(1,safeNum(g.weeklyMinutes)))*100));
...
function bindPageOriginal() {
  ...
  document.querySelectorAll("[data-admin-delete-user]").forEach(b => b.onclick = async () => {
    if (!confirm("Delete this student and ALL of their academic data?")) return;
    try {
      await api(`/api/admin/users/${b.dataset.adminDeleteUser}`, { method: "DELETE" });
      await loadAdmin();
      render();
      toast("Student and related data deleted.");
    } catch (e) { toast(e.message); }
  });
  document.querySelectorAll("[data-action]").forEach(b => b.onclick = () => openForm(b.dataset.action));
  document.querySelectorAll("[data-page-jump]").forEach(b => b.onclick = () => { state.page = b.dataset.pageJump; render(); });
  document.querySelectorAll("[data-delete]").forEach(b => b.onclick = () => removeItem(b.dataset.delete,b.dataset.id));
  document.querySelectorAll("[data-toggle]").forEach(b => b.onclick = () => toggleItem(b.dataset.toggle,b.dataset.id));
  document.querySelectorAll("[data-pin]").forEach(b => b.onclick = () => pinSubject(b.dataset.id));
  document.querySelectorAll("[data-edit]").forEach(b => b.onclick = () => openForm("edit-"+b.dataset.edit,b.dataset.id));
  const plannerForm=$("plannerForm");
  if(plannerForm) plannerForm.onsubmit=e=>{
    e.preventDefault();
  ...
  document.querySelectorAll("[data-plan-done]").forEach(cb=>cb.onchange=()=>{
    const saved=loadSavedPlan();
    if(!saved) return;
    const [di,si]=cb.dataset.planDone.split(":").map(Number);
    if(saved.plan[di]?.sessions[si]) saved.plan[di].sessions[si].done=cb.checked;
    savePlan(saved);
    render();
  });
  const clearPlan=$("clearPlan");
  if(clearPlan) clearPlan.onclick=()=>{
    if(confirm("Clear your current study plan?")) { localStorage.removeItem(plannerStorageKey()); render(); }
  };
  ...
  function updateAttendanceCalc() {
    ...
    const row = state.data.attendance.find(x => x._id === attCalcSubject.value);
    const target = Math.min(99, Math.max(50, Number(attCalcTarget?.value || 80)));
    if (!row) {
    ...
    if (total === 0) {
      attCalcResult.innerHTML = "<b>No classes recorded yet.</b><span>Add attendance data first.</span>";
      return;
    }
    ...
    if (current >= target) {
      ...
      attCalcResult.innerHTML =
        `<b>✅ Target reached</b><span>${esc(row.subject)} is at ${current}%. You can miss approximately <strong>${Math.max(0,maxAbsences)}</strong> more class${maxAbsences===1?"":"es"} and remain at ${target}% or above.</span>`;
      return;
    }

    const needed = Math.ceil((target * total / 100 - attended) / (1 - target / 100));
    const future = attended + needed;
    const futureTotal = total + needed;
    const futurePct = pct(future, futureTotal);

    attCalcResult.innerHTML =
      `<b>⚠️ Attend the next ${needed} class${needed===1?"":"es"} consecutively</b>
  ...
  if(pf) pf.onsubmit=async e=>{e.preventDefault();try{state.user=await api("/api/me",{method:"PUT",body:JSON.stringify({name:$("pName").value,college:$("pCollege").value,branch:$("pBranch").value,semester:$("pSemester").value,rollNo:$("pRoll").value})});$("avatar").textContent=state.user.name[0].toUpperCase();toast("Profile updated");render()}catch(err){toast(err.message)}};
  if($("profilePdf")) $("profilePdf").onclick=()=>$("pdfBtn").click();
  if($("profileBackup")) $("profileBackup").onclick=()=>$("backupBtn").click();
}


function bindPhase713() {
  const prepSel=$("prepExamSelect");
  if(prepSel) prepSel.onchange=()=>{state._selectedExamPrep=prepSel.value;render();};
  document.querySelectorAll("[data-prep-topic]").forEach(cb=>cb.onchange=()=>{
    const examId=$("prepExamSelect")?.value; if(!examId)return;
    const exam=state.data.exams.find(e=>e._id===examId); const p=calculateExamPrep(exam);
    p.topics[Number(cb.dataset.prepTopic)].done=cb.checked; savePrep(examId,p); render();
  });
  const addTopic=$("addTopic");
  if(addTopic) addTopic.onclick=()=>{
    const examId=$("prepExamSelect").value, exam=state.data.exams.find(e=>e._id===examId), p=calculateExamPrep(exam), val=$("newTopic").value.trim();
    if(!val)return toast("Enter a topic."); p.topics.push({id:`t${Date.now()}`,name:val,done:false,important:false}); savePrep(examId,p); render();
  };
  const savePrepBtn=$("savePrep");
  if(savePrepBtn) savePrepBtn.onclick=()=>{
    const examId=$("prepExamSelect").value, exam=state.data.exams.find(e=>e._id===examId), p=calculateExamPrep(exam);
    p.questions=safeNum($("prepQuestions").value); p.notes=$("prepNotes").value; savePrep(examId,p); toast("Exam preparation saved."); render();
  };
  const sendPlanner=$("sendPrepToPlanner");
  if(sendPlanner) sendPlanner.onclick=()=>{state.page="planner"; render(); setTimeout(()=>{const x=$("plannerExam"); if(x){x.value=$("prepExamSelect").value;}},0);};
  const markAll=$("markAllRead");
  if(markAll) markAll.onclick=()=>{const n=ensureNotifications().map(x=>({...x,read:true}));setLocal("notifications",n);render();};
  document.querySelectorAll("[data-notification-read]").forEach(b=>b.onclick=()=>{const n=ensureNotifications();n[Number(b.dataset.notificationRead)].read=true;setLocal("notifications",n);render();});
  const saveGoalsBtn=$("saveGoals");
  if(saveGoalsBtn) saveGoalsBtn.onclick=()=>{const g=goalsData();g.dailyMinutes=safeNum($("goalDaily").value,120);g.weeklyMinutes=safeNum($("goalWeekly").value,600);saveGoals(g);toast("Goals saved.");render();};
  const logStudy=$("logStudy");
  if(logStudy) logStudy.onclick=()=>{const g=goalsData(),mins=safeNum($("goalMinutes").value,0),today=new Date().toISOString().slice(0,10);if(mins<=0)return toast("Enter study minutes.");if(g.lastStudyDate){const last=new Date(g.lastStudyDate+"T00:00:00"),now=new Date(today+"T00:00:00"),diff=Math.round((now-last)/86400000);if(diff===1)g.streak=(g.streak||0)+1;else if(diff>1)g.streak=1;}else g.streak=1;g.lastStudyDate=today;g.completedMinutes=(g.completedMinutes||0)+mins;g.history=(g.history||[]).concat({date:today,minutes:mins}).slice(-90);saveGoals(g);toast(`Logged ${mins} minutes.`);render();};
  const adminSearch=$("adminSearch");
  if(adminSearch) adminSearch.oninput=()=>{state.admin.search=adminSearch.value;render();setTimeout(()=>{const el=$("adminSearch");if(el){el.focus();el.setSelectionRange(el.value.length,el.value.length);}},0);};
  const pfw=$("passwordForm");
  if(pfw) pfw.onsubmit=async e=>{e.preventDefault();try{await api("/api/auth/change-password",{method:"PUT",body:JSON.stringify({currentPassword:$("currentPassword").value,newPassword:$("newPassword").value})});toast("Password changed. Please log in again.");setTimeout(()=>logout(false),800);}catch(err){toast(err.message)}};
}

function bindPage() { ensureNotifications(); bindPageOriginal(); bindPhase713(); }

function openForm(kind,id) {
  let type = kind.startsWith("edit-") ? kind.slice(5) : kind.replace("add-","");
  ...
  form.onsubmit = async e => {
    ...
    try {
      ...
      if (existing) {
        await api(`/api/${type}/${id}`, { method:"PUT", body:JSON.stringify(data) });
      } else {
        await api(`/api/${type}`, { method:"POST", body:JSON.stringify(data) });
      }
      ...
      if (submit) { submit.disabled = false; submit.textContent = type === "subjects" ? "Save Subject" : type === "assignments" ? "Save Assignment" : type === "exams" ? "Save Exam" : type === "timetable" ? "Save Class" : type === "notes" ? "Save Note" : "Save Attendance"; }
    }
  };
}

async function refresh(){state.data=await api("/api/data");if(state.user?.role === "admin") await loadAdmin();render()}
async function removeItem(type,id){if(!confirm("Delete this record?"))return;try{await api(`/api/${type}/${id}`,{method:"DELETE"});await refresh();toast("Deleted")}catch(e){toast(e.message)}}
async function toggleItem(type,id){const item=state.data[type].find(x=>x._id===id);try{await api(`/api/${type}/${id}`,{method:"PUT",body:JSON.stringify({completed:!item.completed})});await refresh();toast(item.completed?"Marked pending":"Marked complete")}catch(e){toast(e.message)}}
async function pinSubject(id){const item=state.data.subjects.find(x=>x._id===id);await api(`/api/subjects/${id}`,{method:"PUT",body:JSON.stringify({pinned:!item.pinned})});await refresh()}
...
views.admin = function(){
  if(state.user?.role!=="admin") return `<div class="card">${empty("Admin access required.")}</div>`;
  const s=state.admin.stats||{}, q=(state.admin.search||"").toLowerCase();
  const users=state.admin.users.filter(u=>[u.name,u.email,u.branch,u.semester,u.rollNo].join(" ").toLowerCase().includes(q));
  ...
  } else if(state.admin.tab==="subjects") content=adminRecordTable(rec.subjects,["Subject","Student","Code","Faculty","Deadline","Status"],x=>`<td><b>${esc(x.name)}</b></td><td>${esc(x.userId?.name||"-")}</td><td>${esc(x.code||"-")}</td><td>${esc(x.faculty||"-")}</td><td>${formatDate(x.deadline)}</td><td>${x.completed?"Completed":"Pending"}</td>`);
  else if(state.admin.tab==="assignments") content=adminRecordTable(rec.assignments,["Assignment","Student","Subject","Due","Priority","Status"],x=>`<td><b>${esc(x.title)}</b></td><td>${esc(x.userId?.name||"-")}</td><td>${esc(x.subject||"-")}</td><td>${formatDate(x.dueDate)}</td><td>${esc(x.priority)}</td><td>${x.completed?"Completed":"Pending"}</td>`);
  else if(state.admin.tab==="attendance") content=adminRecordTable(rec.attendance,["Subject","Student","Attended","Total","Percentage"],x=>`<td><b>${esc(x.subject)}</b></td><td>${esc(x.userId?.name||"-")}</td><td>${x.attended}</td><td>${x.total}</td><td><b>${pct(x.attended,x.total)}%</b></td>`);
  else content=adminRecordTable(rec.exams,["Subject","Student","Type","Date","Time","Room"],x=>`<td><b>${esc(x.subject)}</b></td><td>${esc(x.userId?.name||"-")}</td><td>${esc(x.examType||"-")}</td><td>${formatDate(x.date)}</td><td>${esc(x.time||"-")}</td><td>${esc(x.room||"-")}</td>`);
